...
<head>
    ...
    <style>
        ...
        .painel-card { border-radius: 18px; box-shadow: 0 6px 32px 0 #00000018; }
        .painel-badge { font-size: 1.1rem; border-radius: 1em; padding: 0.7em 1.4em; }
        .table thead th { vertical-align: middle; font-size: 1.02rem; }
        .card-title { font-size: 2.2rem; letter-spacing: 1px; }
        .table-responsive { border-radius: 1rem; overflow: hidden; background: #fff; }
        .painel-group-title { font-size: 1rem; font-weight: 700; color: #7c7c7c; letter-spacing: .5px; margin-bottom: .5em;}
    </style>
</head>
<body class="bg-light">
...
<div class="container py-4">
    <div class="card painel-card border-0 mb-5">
        <div class="card-body">
            ...
            <div class="d-flex flex-wrap gap-4 painel-actions mb-4">

                {{-- Grupo: Relatórios --}}
                <div>
                    <div class="painel-group-title">Relatórios</div>
                    <a href="{{ route('relatorio') }}" class="btn btn-outline-primary shadow-sm w-100 mb-2">
                        📊 Relatório
                    </a>
                    <a href="{{ route('classificacao') }}" class="btn btn-outline-dark shadow-sm w-100">
                ...
                {{-- Grupo: Entrevista --}}
                <div>
                    <div class="painel-group-title">Entrevista</div>
                    <a href="{{ route('entrevista.form') }}" class="btn btn-outline-dark shadow-sm w-100 mb-2">
                        🎤 Lançar Nota da Entrevista
                    </a>
                    <a href="{{ route('recursos-entrevista.admin') }}" class="btn btn-outline-primary shadow-sm w-100">
                ...
                {{-- Usuário --}}
                <div>
                    <div class="painel-group-title">Usuário</div>
                    <form action="{{ route('logout') }}" method="POST" class="d-block">
                        @csrf
                        <button class="btn btn-outline-danger shadow-sm w-100">Sair</button>
                    </form>
                </div>
            </div>
            ...
            <form method="GET" class="row g-3 mb-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="nome_completo" value="{{ request('nome_completo') }}" class="form-control shadow-sm" placeholder="Digite o nome completo">
                </div>
                <div class="col-md-2">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-primary">
                        <tr>
                            ...
                            <th>Data</th>
                            <th>PDFs</th>
                            <th style="min-width: 130px;">Entrevista</th>
                    ...
                    <tbody>
                        @foreach($inscricoes as $insc)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('entrevista.salvar', $insc->id) }}" method="POST" class="d-flex gap-2 align-items-center">
                                        @csrf
                                        <input type="number" name="pontuacao_entrevista" class="form-control form-control-sm" style="width: 85px;"
                                            value="{{ $insc->pontuacao_entrevista }}" min="0" max="1000" required>
                                        <button class="btn btn-outline-success btn-sm" title="Salvar"><span>💾</span></button>
                                    </form>
                                </td>
                                <td>
                                    <form action="{{ route('avaliar.salvar', $insc->id) }}" method="POST" class="d-flex flex-column gap-2">
                                        ...
                                        <select name="motivo_indeferimento" class="form-select motivo-select" style="{{ $insc->status_avaliacao == 'Indeferido' ? '' : 'display: none;' }}">
                                            ...
                                            <!-- (Demais opções aqui, igual ao seu código original) -->
                                        </select>
