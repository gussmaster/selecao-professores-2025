...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4>Recursos contra Pontuação</h4>
        <div>
            <a href="{{ route('recursos-pontuacao.export.csv') }}" class="btn btn-outline-success me-2">
                ⬇️ Exportar CSV
            </a>
            <a href="{{ route('recursos-pontuacao.export.pdf') }}" class="btn btn-outline-danger">
                ⬇️ Exportar PDF
            </a>
        </div>
    </div>

    <form method="GET" action="{{ route('recursos-pontuacao.admin') }}" class="d-flex align-items-center mb-3 gap-2 flex-wrap">
        <input type="text" name="cpf" class="form-control" placeholder="Filtrar por CPF" value="{{ request('cpf') }}" style="max-width:180px;">
        <button class="btn btn-primary">Filtrar</button>
        @if(request('cpf'))
            <a href="{{ route('recursos-pontuacao.admin') }}" class="btn btn-outline-secondary">Limpar Filtro</a>
        @endif
    </form>
            ...
            <tbody>
                @forelse($recursos as $recurso)
                    <tr>
                        ...
                        <td>{{ $recurso->cpf ?? '-' }}</td>
                        <td>{{ $recurso->cargo ?? '-' }}</td>
                        <td><b>{{ $recurso->pontuacao_atual }}</b></td>
                        <td>{{ $recurso->motivo ?? '-' }}</td>
                        <td>{{ $recurso->status_analise ?? '-' }}</td>
                        <td>
                            @if($recurso->status_analise === 'Aceito')
                                <form method="POST" action="{{ route('recursos-pontuacao.analise', $recurso->id) }}">
                                    @csrf
                        ...
                        <td class="text-center">
                            <form method="POST" action="{{ route('recursos-pontuacao.analise', $recurso->id) }}" class="d-flex flex-column align-items-center gap-1">
                                @csrf
